...
snr_dB = 30;    % Total power in dB
c = 2.2;            % Pathloss component
No = 1;             % Noise power
iterations = 500;    % Number of iterations for Monte Carlo simulation
...
plot(d_je1, C_RIS1_nojam1, '-m^', 'DisplayName', 'user 1 sr nojamming', 'LineWidth', 2.2, 'Color', 'g');

plot(d_je1, C_RIS1_unknownjam1, '-m^', 'DisplayName', 'user 1 sr unknownjamming', 'LineWidth', 2.2, 'Color', 'r');
 
plot(d_je1, C_RIS1_knownjam1, '-m^', 'DisplayName', 'user 1 sr knownjamming', 'LineWidth', 2.2, 'Color', 'k');

xlabel('d_je1', 'FontWeight', 'bold');
ylabel('Secrecy Rate (bits/sec/Hz)', 'FontWeight', 'bold');
title('secrecy rate of user 1 vs d_je1 in non collabrative case')
grid on;
