...
for a_index = 1:length(a1_values)
    ...
    for jj = 1:iterations
         ...
         for ii = 1:length(snr_dB)
             ...
             Pt = 0.85 * Ps;              % Power for users
             Pj = 0.15* Ps;              % Power for jamming
...
plot(snr_dB, C_RIS1_nojam2, '-b|', 'DisplayName', "user 2 sr nojamming", 'LineWidth', 2.2, 'Color', 'g');
plot(snr_dB, C_RIS1_unknownjam2, '-b|', 'DisplayName', "user 2 sr unknownjamming", 'LineWidth', 2.2, 'Color', 'r');
 plot(snr_dB, C_RIS1_knownjam2, '-b|', 'DisplayName', "user 2 sr knownjamming", 'LineWidth', 2.2, 'Color', 'k');

xlabel('SNR (dB)', 'FontWeight', 'bold');
ylabel('Secrecy Rate (bits/sec/Hz)', 'FontWeight', 'bold');
title('secrecy rate vs snr at user 2 in non collabrative condition')
grid on;
